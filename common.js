 ...
 * Public interface
 */
function moveToPoint(lat, lon, isAnimated, margin) {
  // Set margin-bottom for mobile overlay
  mapInstance.margin.setDefaultMargin(margin);
...
function moveToCluster(coords, isAnimated, margin) {
  ymaps.util.requireCenterAndZoom(
    mapInstance.getType(),
    ymaps.util.pixelBounds.fromPoints(coords),
